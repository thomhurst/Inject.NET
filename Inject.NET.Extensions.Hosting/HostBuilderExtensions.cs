...
public static class HostBuilderExtensions
{
    ...
    /// <summary>
    /// Configures an <see cref="IHostApplicationBuilder"/> (including
    /// <c>WebApplicationBuilder</c> and the modern <c>HostApplicationBuilder</c>) to use
    /// an existing Inject.NET service provider as the backing DI container.
    /// </summary>
    /// <param name="builder">The host application builder to configure.</param>
    /// <param name="provider">The pre-built Inject.NET service provider.</param>
    /// <returns>The host application builder for fluent chaining.</returns>
    /// <example>
    /// <code>
    /// await using var provider = await MyServiceProvider.BuildAsync();
    /// var builder = WebApplication.CreateBuilder(args);
    /// builder.UseInjectNet(provider);
    /// var app = builder.Build();
    /// </code>
    /// </example>
    public static IHostApplicationBuilder UseInjectNet(this IHostApplicationBuilder builder, IServiceProvider provider)
    {
        builder.ConfigureContainer(new InjectNetServiceProviderFactory(provider));
        return builder;
    }
}
